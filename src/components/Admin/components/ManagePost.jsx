...
import {
    getAllPostsInactive, getDetailPostById, banPost, unbanPost, inactivePost, activePost
} from "@/fetchData/Post";
...
const ManagePostAdmin = () => {
    ...
    const handleStatusChange = (e) => {
        ...
        if (status.toUpperCase() === "ALL".toUpperCase()) {
            ...
            filtered = posts.filter(Post => Post.statusCode.toUpperCase() === status.toUpperCase());
        }
    ...
    const handleUnban = async (note) => {
        try {
            const res = await unbanPost(currentPostDetail.id, note, currentPostDetail.userId);
            console.log("res.data: ", res.data.errCode);
            if (res.data.errCode === 0) {
            ...
            <Table className="table-auto w-full">
                ...
                <TableBody>
                    {currentposts.map((post, index) => (
                        <TableRow key={post.id} onClick={() => handleOpenUpdateModal(post.id)} className="cursor-pointer hover:bg-slate-300">
                                ...
                                <dl>
                                    ...
                                    <div className="pt-6">
                                        <div className="flex justify-end">
                                            {currentPostDetail.statusCode.toUpperCase() === 'PENDING'.toUpperCase() && (
                                                <>
                                                    <button
                                            ...
                                            {currentPostDetail.statusCode.toUpperCase() === 'ACTIVE'.toUpperCase() && (
                                            ...
                                            {currentPostDetail.statusCode.toUpperCase() === 'INACTIVE'.toUpperCase() && (
                                                <button
                                                    onClick={handleActive}
                                            ...
                                            {currentPostDetail.statusCode.toUpperCase() === 'BANNED'.toUpperCase() && (
                                                <button
                                                    onClick={() => {
                                                        setShowConfirm(true);
                                                        setAction("unban");
                                                ...
                                                <DialogContent className="max-w-sm">
                                                    ...
                                                    <div className="flex justify-center mt-4">
                                                        <button
                                                            onClick={
                                                                () => {
                                                                    if (action.toUpperCase() === 'ban'.toUpperCase()) { handleBan(note); }
                                                                    if (action.toUpperCase() === 'unban'.toUpperCase()) { handleUnban(note); }
                                                                    if (action.toUpperCase() === 'inactive'.toUpperCase()) { handleInactive(note); }
                                                                    setNote('');
                                                                }
