...
function CompanyPage() {
  ...
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [totalCount, setCounts] = useState(0);
  const [searchParams, setSearchParams] = useSearchParams();
  const [currentPage, setCurrentPage] = useState(Number(searchParams.get('page')) || 1);
  const companiesPerPage = 6;
  ...
  const filterCompanies = (companies, typeCompany) => {
    const upperCaseType = String(typeCompany).toUpperCase(); // Ensure typeCompany is a string
    return companies.filter(company => 
      upperCaseType === 'CATEGORIES' || String(company.typeCompany).toUpperCase() === upperCaseType
    );
  };
  ...
  const fetchCompanies = async (searchKey = '', typeCompany = 'Categories') => {
    try {
      ...
      if (response && response.data && response.data.errCode === 0) {
        ...
        setCounts(allCompanies.length);
        setCompanies(allCompanies);
        const filtered = filterCompanies(allCompanies, typeCompany);
        setFilteredCompanies(filtered);
        
        const uniqueTypes = [...new Set(allCompanies.map(company => String(company.typeCompany).toUpperCase()))];
        setTypes(uniqueTypes.map(type => ({ name: type })));
      } else {
        setError(response.data ? response.data.errMessage : 'Unknown error');
      }
    } catch (error) {
      console.error("Error fetching companies:", error);
      setError("Error fetching data. Please try again later.");
  ...
  useEffect(() => {
    fetchCompanies(filter.company, filter.typeCompany);
  }, [filter.company, filter.typeCompany, currentPage]); // Ensure currentPage is included in the dependency array
